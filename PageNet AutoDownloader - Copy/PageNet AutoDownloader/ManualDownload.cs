            ...
            lock (_object)
            {
                using (System.IO.StreamWriter file = new System.IO.StreamWriter(@AppDomain.CurrentDomain.BaseDirectory + StrDate + "_Logs.txt", true))
                {
                    ...
                    foreach (FTPFiles i in ftpfiles)
                    {
                        ...
                        DTPCheck.Value.Subtract(TimeSpan.FromDays(1)).Day.ToString("00") + @"\" + Grid.Rows[currRow].Cells[0].Value.ToString() 
                        + @"\" + i.FileName.Substring(0, 12).ToString() + ".zip";

                        //File exist code is in the if statement
                        if (File.Exists(@FileLoc) == false)
                        {                            
                            Grid2.Rows.Add(i.FileName.Substring(0, 12).ToString(),0,0,0,0, i.FileBytes, Grid.Rows[currRow].Cells[1].Value.ToString(), Grid.Rows[currRow].Cells[2].Value.ToString(), Grid.Rows[currRow].Cells[3].Value.ToString(), currRow);
